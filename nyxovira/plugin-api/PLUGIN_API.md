...
> Create and validate a Nyxovira plugin for the supplied URL using this documentation. Inspect the real page, scripts, network traffic, and first-party documentation/API before coding; do not invent data. Create `plugin.json` and `browser/download_target.js`, immediately define a lightweight `__nyxoviraChapterPlan`, and resolve all selected content in `__nyxoviraPrepareDownloadPlan` while preserving the exact IDs from `selectedChapterIds`. For protected images, return `{ url, headers, contentType }`; declare `parser` only when real traffic requires a structured API, static headers, encryption, or native downloading. Build the ZIP and test a real work in the [Web Laboratory](tester/). The first chapter must be downloaded completely without skipping pages. If any step is `FAIL` or `BLOCKED`, fix it, build another ZIP, and rerun the test. Claim success only when the JSON has `success: true`, `verdict: PLUGIN_VALID`, `validationScope: complete-first-chapter`, no failure, `temporaryDataReleased: true`, and `stored: false`; for comics, `verifiedPageCount` must match the resolved total and the report must state N of N verified images. Deliver the real files, ZIP, and JSON report. Without network access, state “not validated against the real site.”
