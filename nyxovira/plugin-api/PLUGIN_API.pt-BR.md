...
Mapeie `Referer`, cookies de sessão, token por capítulo/página, user agent e headers estáticos separadamente. A autenticação não torna o plugin limitado ao conteúdo público: ele pode usar uma sessão autorizada já aberta pelo usuário e somente o acesso concedido a essa sessão. Não determine o alcance por texto promocional ou aviso genérico; confirme o comportamento real da sessão autorizada. `request_headers` só transporta valores estáticos do parser; cookies da WebView não devem ser considerados disponíveis no downloader nativo sem um teste real. Nunca solicite, grave, exporte ou inclua credenciais, cookies ou tokens pessoais nos arquivos do plugin, no ZIP ou nos relatórios. Se a sessão não permitir um capítulo, registre-o como indisponível nessa sessão sem inventar resultado.
...
[Abra o Laboratório Web de Plugins](tester/), envie o ZIP e informe a URL exata da obra que precisa ser testada. O serviço escolhe o primeiro e o último capítulo automaticamente e baixa todas as páginas dos dois capítulos em lotes, com quatro conexões e três novas tentativas, usando o mesmo perfil HTTP do downloader Nyxovira. O resultado `PLUGIN_VALID_FOR_TESTED_WORK` vale somente para a obra, os capítulos e o SHA-256 do ZIP registrados no relatório; ele não aprova outra obra, capítulo, ZIP ou instalação anterior.
...
- [ ] O primeiro e o último capítulo reais resolvem páginas ou parágrafos.
...
- [ ] O laboratório web retornou `PLUGIN_VALID_FOR_TESTED_WORK`, `validationScope: exact-work-complete-boundary-chapters`, `requestProfile: nyxovira-android-http-v1`, `packageSha256`, `temporaryDataReleased: true`, `stored: false` e nenhuma etapa `FAIL`.
...
> Crie e valide um plugin Nyxovira para a URL informada seguindo esta documentação. Inspecione a página real, scripts, tráfego de rede e documentação/API oficial antes de programar; não invente dados. Se houver autenticação, não reduza automaticamente o plugin a capítulos públicos: use somente a sessão já autenticada pelo usuário e o acesso concedido a ela, sem solicitar, registrar, exportar ou incluir credenciais, cookies ou tokens pessoais no plugin, ZIP ou relatório. Confirme o comportamento real dessa sessão em vez de inferir o alcance por texto promocional ou aviso genérico; se ela não permitir um capítulo, registre-o como indisponível nessa sessão. Crie `plugin.json` e `browser/download_target.js`, defina imediatamente um `__nyxoviraChapterPlan` leve e resolva todo o conteúdo selecionado em `__nyxoviraPrepareDownloadPlan`, preservando exatamente os IDs de `selectedChapterIds`. Para imagens protegidas, retorne `{ url, headers, contentType }`; declare `parser` apenas quando o tráfego real exigir API estruturada, headers estáticos, criptografia ou download nativo. Gere o ZIP e teste a URL exata da obra no [Laboratório Web](tester/). O primeiro e o último capítulo precisam ser baixados por inteiro, sem pular páginas, com o perfil concorrente do Nyxovira. Se houver `FAIL` ou `BLOCKED`, corrija, gere outro ZIP e repita. Só declare sucesso para essa obra e esse ZIP quando o JSON tiver `success: true`, `verdict: PLUGIN_VALID_FOR_TESTED_WORK`, `validationScope: exact-work-complete-boundary-chapters`, `requestProfile: nyxovira-android-http-v1`, `packageSha256`, nenhuma falha, `temporaryDataReleased: true` e `stored: false`; em quadrinhos, `verifiedPageCount` deve corresponder ao total resolvido e o relatório deve informar N de N imagens. Nunca generalize o resultado para outra obra, capítulo, ZIP ou instalação. Entregue arquivos, ZIP e relatório JSON reais. Sem rede, escreva “não validado contra o site real”.
