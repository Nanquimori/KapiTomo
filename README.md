...
The Hub includes a `Report` action on every published plugin and a dedicated report panel for plugin-code, repository, or catalog-entry violations. The form requires the plugin ID, a valid contact email, a reason of 200 to 2,000 characters containing at least 30 words and 10 different words, and an explicit scope/accuracy confirmation. It prepares a private email for the reporter to review and send to the catalog contact address. Reports are reviewed manually and never hide or remove an entry automatically.
