...
The Hub includes a `Report` action on every published plugin and a dedicated report panel for plugin-code, repository, or catalog-entry violations. The form requires the plugin ID, a valid contact email, a reason written in Portuguese or English with 200 to 2,000 characters and at least 20 words, and an explicit scope/accuracy confirmation. It sends the report privately through FormSubmit and displays success only after the service accepts the submission. Accepted reports receive a stable plugin grouping key and a normalized duplicate fingerprint; the same accepted report cannot be resent from the same browser for 30 days. Duplicate reports are grouped into one review, their count is never treated as evidence, and report quantity never causes a plugin to be hidden or removed.
